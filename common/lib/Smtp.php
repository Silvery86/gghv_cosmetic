...
class Smtp {
	 ...
	 */
	public function __construct(string $server, int $port = 25, int $connectionTimeout = 30, int $responseTimeout = 8, ?string $hostname = null) {
		$this->port              = $port;
		$this->server            = $server;
	 ...
	 * @return Smtp
	 */
	public function setHeader(string $key, $value = null): Smtp {
	...
	public function send(): bool {
		$message      = null;
		$this->socket = fsockopen($this->getServer(), $this->port, $errorNumber, $errorMessage, $this->connectionTimeout);
